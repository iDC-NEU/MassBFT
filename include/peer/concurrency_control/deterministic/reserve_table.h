...
namespace peer::cc {
    ...
    public:
        ...
        // =0; left=right
        // >0; left>right
        static int CompareTID(const proto::tid_type &lhs, const proto::tid_type &rhs) {
        ...
        void reserveRWSets(const proto::KVList &reads,
            ...
            for (const auto &read: reads) {
                ...
                auto exist = [&](TableType::value_type &value) {
                    auto ret = CompareTID(*value.second, *transactionID);
        ...
        [[nodiscard]] Dependency analysisDependent(const proto::KVList &reads,
            ...
            for (const auto &write: writes) {
                writeTable.if_contains_unsafe(write->getKeySV(), [&](const TableType::value_type &v) {
                    if (CompareTID(*v.second, transactionID) < 0) {  // waw dependency
                        dependency.waw = true;
                    }
                });
                if (dependency.waw) {
            ...
            for (const auto &write: writes) {
                readTable.if_contains_unsafe(write->getKeySV(), [&](const TableType::value_type &v) {
                    if (CompareTID(*v.second, transactionID) < 0) {  // war dependency
                        dependency.war = true;
                    }
                });
                if (dependency.war) {
            ...
            for (const auto &read: reads) {
                writeTable.if_contains_unsafe(read->getKeySV(), [&](const TableType::value_type &v) {
                    if (CompareTID(*v.second, transactionID) < 0) {  // raw dependency
                        dependency.raw = true;
                    }
                });
                if (dependency.raw) {
