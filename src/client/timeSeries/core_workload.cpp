...
namespace client::timeSeries {
    ...
    std::unique_ptr<core::NumberGenerator> CoreWorkload::getFieldLengthGenerator(const TimeSeriesProperties &n) {
    ...
    std::unique_ptr<core::Generator<uint64_t>> CoreWorkload::initKeyChooser(const TimeSeriesProperties &n) const {
    ...
    std::unique_ptr<core::NumberGenerator> CoreWorkload::initScanLength(const TimeSeriesProperties &n) {
    ...
    void CoreWorkload::init(const util::Properties &prop) {
        auto n = TimeSeriesProperties::NewFromProperty(prop);
